...
bool search(int rt, int key) {
    ...
    if (data[rt] == key) return true;
    if (key < data[rt])
        return search(left[rt], key);
    else
        return search(right[rt], key);
}
// ==================== 3. 修改 ====================
bool update(int rt, int oldVal, int newVal) {
    ...
    if (oldVal < data[rt])
        return update(left[rt], oldVal, newVal);
    else
        return update(right[rt], oldVal, newVal);
}
// ==================== 4. 删除（辅助） ====================
// 找最小值节点
int findMin(int rt) {
    while (left[rt] != NULL_NODE)
        rt = left[rt];
    return rt;
}
// 删除节点
int remove(int rt, int val) {
    ...
    if (val < data[rt]) {
        left[rt] = remove(left[rt], val);
    }
    else if (val > data[rt]) {
        right[rt] = remove(right[rt], val);
    }
    else {
        // 无左孩子
        if (left[rt] == NULL_NODE)
            return right[rt];
        // 无右孩子
        if (right[rt] == NULL_NODE)
            return left[rt];
        // 两个孩子：用后继替换
        int minNode = findMin(right[rt]);
        data[rt] = data[minNode];
        right[rt] = remove(right[rt], data[minNode]);
    }
    return rt;
}
// ==================== 中序遍历（验证BST） ====================
